class TocStateManager extends TocSheet {
  ...
  */
  ...
  arraysAreSimilar(arr1 = this.previousState, arr2 = this.currentState) {
    ...
    const differencesFromPrev = [];
    const differencesFromCurr = [];
    const allElementsInArr2 = arr1.every(element => {
      if(!arr2.includes(element)){
        differencesFromPrev.push(element)
      }
      return arr2.includes(element)
      });
    const allElementsInArr1 = arr2.every(element => {
      if(!arr2.includes(element)){
        differencesFromCurr.push(element)
      }
      return arr1.includes(element)
      });

    console.log(`Previous found differences: ${differencesFromPrev} vs. Current found differences: ${differencesFromCurr}`)
  ...
  logStates() {
    const prevSheetNames = this.getPreviousState();
    const currSheetNames = this.getCurrentState();
    console.log(`PREV SHEET NAMES: Total ${prevSheetNames.length}, ${prevSheetNames}\nCURR SHEET NAMES: Total ${currSheetNames.length},
        ${currSheetNames}`);
  }
